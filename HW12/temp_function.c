...
{
    printf("===================================\n");
    printf("Annual temperature statistics\n");
    printf("average t=%3d, min t=%3d, max t=%3d\n",
...
{
    struct sensor *sa = (struct sensor*)a;
    struct sensor *sb = (struct sensor*)b;
...
unsigned int DateToInt(struct sensor *info) { return info->year << 16 | info->month << 8 | info->day; } // упорядочивающую его по дате
...
{
    return ((struct sensor *)a)->t - ((struct sensor *)b)->t;
}